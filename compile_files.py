...
def compile_files(data_sol_source_project_dir_path, data_ast_json_project_dir_path):
    ...
    for file_name in os.listdir(data_sol_source_project_dir_path):
        ...
        full_compile_file_path = f'{data_sol_source_project_dir_path}/{file_name}'
        # 保存的json文件的全路径,如"/home/xjj/AST-GNN/data/AST_json/project_name/file_name.json",这里代表了文件的名字，已经到了文件级别
        full_compile_target_path = f'{data_ast_json_project_dir_path}/{file_name.replace(".sol", ".json")}'
        # 保存所有的版本号
        versions = []
