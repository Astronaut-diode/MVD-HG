...
def line_classification_train():
    try:
        ...
        for epoch in range(config.epoch_size):
            ...
            utils.tip(f"epoch{epoch + 1}.结束，一共训练了{count}张图, 总损失值为: {train_total_loss}，学习率为:{optimizer.state_dict()['param_groups'][0]['lr']}，本次耗时{epoch_end_time - epoch_start_time}")
