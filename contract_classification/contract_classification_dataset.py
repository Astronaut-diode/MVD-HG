...
class contract_classification_dataset(Dataset):
        ...
        for project_full_path in self.raw_file_names:
            ...
            for now_dir, child_dirs, child_files in os.walk(project_full_path.replace("raw", "AST_json")):
                for file_name in child_files:
                    ...
                    contract_buggy_record = self.get_contract_buggy_record(os.path.join(now_dir.replace("AST_json", "raw"), file_name.replace(".json", "")))
    ...
    # 这个方法是用来获取节点的json文件中的contract_buggy_record，这记录了当前文件中的contract是否为脆弱的。
    @staticmethod
    def get_contract_buggy_record(project):
        # 先获取节点文件的路径
        node_file_path = f"{project}_node.json"
        # 根据路径获取对应的句柄
        node_file_handle = open(node_file_path, 'r')
        # 通过句柄，读取json文件中的内容
        node_content = json.load(node_file_handle)
        x = node_content["contract_buggy_record"]
        # 已经读取完毕，所以需要关闭句柄对象
        node_file_handle.close()
        return x
