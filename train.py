...
def train():
    ...
    dfg_dataset = ASTGNNDataset(config.data_dir_path, "DFG")
    ...
    dfg_data_loader = DataLoader(dataset=dfg_dataset, batch_size=config.batch_size, shuffle=False, drop_last=False)
    model = ASTGNNModel()
    # 将模型转化为在指定设备上运行的类型。
    ...
    for epoch in range(config.epoch_size):
        ...
        for index, batch in enumerate(zip(ast_data_loader, cfg_data_loader, dfg_data_loader)):
            # 先拆出batch
            ast_batch = batch[0]
            cfg_batch = batch[1]
            dfg_batch = batch[2]
            ...
            dfg_batch = dfg_batch.to(device)
            # 经典的五步计算
            optimizer.zero_grad()
            # 将两个batch的节点，边还有batch信息都传进去
            output = model(ast_batch.x, ast_batch.edge_index, ast_batch.batch, cfg_batch.x, cfg_batch.edge_index, cfg_batch.batch, dfg_batch.x, dfg_batch.edge_index, dfg_batch.batch)
