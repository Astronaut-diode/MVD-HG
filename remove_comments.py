...
def remove_comments(data_sol_source_project_dir_path):
    tmp_file_name = f'{data_sol_source_project_dir_path}/tmp.sol'
    for file_name in os.listdir(data_sol_source_project_dir_path):
        # 如果是临时文件需要跳过该文件的处理
        if file_name == "tmp.sol":
            continue
        # 源文件的全路径
        file = f'{data_sol_source_project_dir_path}/{file_name}'
        # 打开源文件和一个新文件，将源文件中的内容抄到新的文件当中去。
        with open(file, 'r') as origin_file, open(tmp_file_name, 'w') as tmp_file:
