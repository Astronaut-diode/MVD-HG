...
cmd = f"/root/anaconda3/envs/lunikhod/bin/python3 /root/MVD-HG/main.py --run_mode predict_line --data_dir_name {str(dest_dir).split('/')[-2]} --attack_type_name {attack_type} --coefficient 0.1 --target_dir {source_dir.split('/')[-1]} --target_file {filename}"
print(cmd)
# 创建一个子进程并立即返回
...
# 输出命令执行结果
file = open(dest_dir + "/res_line.json", 'r')
