...
def classification_of_documents():
    ...
    print(f"疑似重入漏洞的数量为{suspected_reen}")
    print(f"时间戳漏洞的数量为{time}")
    print(f"疑似时间戳漏洞的数量为{suspected_time}")
    print(f"溢出漏洞的数量为{arit}")
    print(f"疑似溢出漏洞的数量为{suspected_arit}")
    print(f"危险调用漏洞的数量为{dele}")
    print(f"疑似危险调用漏洞的数量为{suspected_dele}")
    print("确认有漏洞的文件都已经全部移入到对应的漏洞文件夹中。")
