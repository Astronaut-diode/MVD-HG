            ...
            for function_definition_node in project_node_dict["FunctionDefinition"]:
                ...
                if function_definition_node.parent == contract_node and len(function_definition_node.control_childes):
                    ...
                    if len(has_reentry_flag):
                        reentry_flag = True
                        break
    utils.success(f"{file_name}重入标签已经检测完毕。")
    # 最终返回这个控制的变量即可。
    if reentry_flag:
        ...
        while argument_node is not None:
            ...
            if argument_node.node_type == "Literal":
                ...
                for param in params[::-1]:
                    ...
                    if param.node_type == "Literal" and ((param.parent.node_type == "Assignment" and param.parent.attribute["operator"][0] == "=") or (param.parent.node_type == "VariableDeclarationStatement" and len(param.parent.childes) == 2)) and param == param.parent.childes[1] and param.attribute["src_code"] == argument_node.attribute["src_code"] and param != argument_node:
                        # 可以直接得出起点
            ...
            # 如果是一个简单的变量
                ...
                for param in params[::-1]:
                    ...
                    if param.node_type == "IndexAccess" and ((param.parent.node_type == "Assignment" and param.parent.attribute["operator"][0] == "=") or (param.parent.node_type == "VariableDeclarationStatement" and len(param.parent.childes) == 2)) and has_same_structure(argument_node, param) and param == param.parent.childes[1]:
                        # 可以直接得出起点
                        start_node = param.parent.childes[0]
                        # 根据起点我们可以推断出最后一次使用这个变量的位置,将这个推断出的位置,再一次当作新的转账金额进行判断即可.
                        for last_appear_position in params[::-1]:
                    ...
                    elif param.node_type == "IndexAccess" and ((param.parent.node_type == "Assignment" and param.parent.attribute["operator"][0] == "=") or (param.parent.node_type == "VariableDeclarationStatement" and len(param.parent.childes) == 2)) and has_same_structure(argument_node, param) and param == param.parent.childes[0]:
                        # 遍历由param.parent.childes[1]得出对应的data_parents
                        for source_node_of_tmp_variable in params[::-1]:
                            # 如果已经找到了等价元素，那就跳出循环，没有必要继续了
                            if have_equal_variable:
                ...
                for param in params[::-1]:
                    ...
                    if param.node_type == "BinaryOperation" and ((param.parent.node_type == "Assignment" and param.parent.attribute["operator"][0] == "=") or (param.parent.node_type == "VariableDeclarationStatement" and len(param.parent.childes) == 2)) and has_same_structure(argument_node, param) and param == param.parent.childes[1]:
                        # 可以直接得出起点
                        start_node = param.parent.childes[0]
                        # 根据起点我们可以推断出最后一次使用这个变量的位置,将这个推断出的位置,再一次当作新的转账金额进行判断即可.
                        for last_appear_position in params[::-1]:
                            ...
                            if last_appear_position in start_node.data_childes and is_assignment_left(last_appear_position) is False:
                                ...
                                break
                    # 当结果被作为了第一个子节点的时候的处理方式,也就是金额 = tmp
                    elif param.node_type == "BinaryOperation" and ((param.parent.node_type == "Assignment" and param.parent.attribute["operator"][0] == "=") or (param.parent.node_type == "VariableDeclarationStatement" and len(param.parent.childes) == 2)) and has_same_structure(argument_node, param) and param == param.parent.childes[0]:
                        # 遍历由param.parent.childes[1]得出对应的data_parents
                        for source_node_of_tmp_variable in params[::-1]:
                            # 如果已经找到了等价元素，那就跳出循环，没有必要继续了
                            if have_equal_variable:
                ...
                for param in params[::-1]:
                    ...
                    if param.node_type == "MemberAccess" and ((param.parent.node_type == "Assignment" and param.parent.attribute["operator"][0] == "=") or (param.parent.node_type == "VariableDeclarationStatement" and len(param.parent.childes) == 2)) and has_same_structure(argument_node, param) and param == param.parent.childes[1]:
                        # 可以直接得出起点
                        start_node = param.parent.childes[0]
                        # 根据起点我们可以推断出最后一次使用这个变量的位置,将这个推断出的位置,再一次当作新的转账金额进行判断即可.
                        for last_appear_position in params[::-1]:
                            ...
                            if last_appear_position in start_node.data_childes and is_assignment_left(last_appear_position) is False:
                                ...
                                break
                    # 当结果被作为了第一个子节点的时候的处理方式,也就是金额 = tmp
                    elif param.node_type == "MemberAccess" and ((param.parent.node_type == "Assignment" and param.parent.attribute["operator"][0] == "=") or (param.parent.node_type == "VariableDeclarationStatement" and len(param.parent.childes) == 2)) and has_same_structure(argument_node, param) and param == param.parent.childes[0]:
                        # 遍历由param.parent.childes[1]得出对应的data_parents
                        for source_node_of_tmp_variable in params[::-1]:
                            # 如果已经找到了等价元素，那就跳出循环，没有必要继续了
                            if have_equal_variable:
